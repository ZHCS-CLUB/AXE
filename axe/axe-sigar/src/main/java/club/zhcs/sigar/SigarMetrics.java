 ...
 *
 */
public class SigarMetrics implements MeterBinder {

	Log logger = Logs.get();

	// public Collection<Metric<?>> metrics() {
	// List<Metrics<?>> metrics = new ArrayList<MetricsEndpoint<?>>();
	// try {
	// final Sigar sigar = SigarFactory.load();
	// // CPU
	// metrics.add(new Metrics<Number>("cpu.total", sigar.getCpu().getTotal()));
	...
	// metrics.add(new Metric<Number>("cpu.usage", sigar.getCpuPerc().getCombined()
	// * 100));
	// // MEM
	// metrics.add(new Metric<Number>("mem.free", sigar.getMem().getFree()));
	// metrics.add(new Metric<Number>("mem.actual.free",
	// sigar.getMem().getActualFree()));
	// metrics.add(new Metric<Number>("mem.actual.used",
	// sigar.getMem().getActualUsed()));
	// metrics.add(new Metric<Number>("mem.free.percent",
	// sigar.getMem().getFreePercent()));
	...
	// metrics.add(new Metric<Number>("mem.user.percent",
	// sigar.getMem().getUsedPercent()));
	// // NET
	// metrics.add(new Metric<Number>("tcp.active.opens",
	// sigar.getTcp().getActiveOpens()));
	// metrics.add(new Metric<Number>("tcp.attempt.fails",
	// sigar.getTcp().getAttemptFails()));
	// metrics.add(new Metric<Number>("tcp.curre.stab",
	// sigar.getTcp().getCurrEstab()));
	// metrics.add(new Metric<Number>("tcp.estab.resets",
	...
	// metrics.add(new Metric<Number>("tcp.passive.opens",
	// sigar.getTcp().getPassiveOpens()));
	// metrics.add(new Metric<Number>("tcp.retrans.segs",
	...
	// metrics.add(new Metric<Number>("swap.page.out",
	// sigar.getSwap().getPageOut()));
	...
	// metrics.add(new Metric<Number>("swap.usage", sigar.getSwap().getUsed() * 100
	// / sigar.getSwap().getTotal()));
	//
	// // JVM
	//
	...
	public void bindTo(MeterRegistry registry) {
		final Sigar sigar = SigarFactory.load();
		try {
			registry.gauge("cpu.total", sigar.getCpu().getTotal());
			registry.gauge("cpu.total", sigar.getCpu().getTotal());
		} catch (SigarException e) {
			e.printStackTrace();
		}
