...
public abstract class AbstractSinger implements Signer {
	 ...
	 * @return 签名字符串
	 */
	public String sign(AppsecretFetcher fetcher, String appKey, String timestamp, String gateway, String nonce,
			String dataMate) {
	...
	public String sign(HttpServletRequest request, AppsecretFetcher fetcher) {
		return sign(fetcher, request.getHeader(ROPConfig.APP_KEY_KEY), request.getHeader(ROPConfig.TS_KEY),
				request.getHeader(ROPConfig.METHOD_KEY), request.getHeader(ROPConfig.NONCE_KEY), getDataMate(request));
	}
